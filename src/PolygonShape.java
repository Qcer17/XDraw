...
public class PolygonShape extends Shape{
	...
	private void drawPolygon(Graphics g) {
		...
		for(int i=0;i<points.size()-1;i++) {
			LineShape.drawLine(g, (int)points.get(i).getX(), (int)points.get(i).getY(), 
					(int)points.get(i+1).getX(), (int)points.get(i+1).getY(),true,pointsOnFrame);
		}
		if(!points.get(0).equals(points.get(points.size()-1))) {
			LineShape.drawLine(g, (int)points.get(points.size()-1).getX(), (int)points.get(points.size()-1).getY(), 
					(int)points.get(0).getX(), (int)points.get(0).getY(),true,pointsOnFrame);
		}
	}
	...
	public void addNumPoints() {
		...
		tFloatingIcon.setVisible(false);
	...
	public PolygonShape(int x1,int y1,int x2,int y2,Color color,Color color2, PaintArea paintArea) {
		super();
		bdotsX[0]=-1;
		addPoint(x1, y1);
		addPoint(x2, y2);
	...
	//	paintArea.add(rotateCenter);
		...
		assistDot.setVisible(false);
		lastDot=new FloatingIcon("square.png");
	...
	public void updateIcons() {
		...
		for(FloatingIcon point:pointIcons) {
			paintArea.remove(point);
			paintArea.add(point,0);
		}
	}
	...
	void fillPolygon(Graphics g) {
		...
		int startY=Math.max(LineShape.Y1,pointsOnFrame.get(0).getY());
		int endY=Math.min(LineShape.Y2,pointsOnFrame.get(pointsOnFrame.size()-1).getY());
		int id=1;
		boolean odd=false;
		int startX=0,endX=0;
		for(int y=startY+1;y<endY;y++) {
//			odd=false;
//			while(pointsOnFrame.get(id).getY()!=y)id++;
//			while(pointsOnFrame.get(id).getY()==y) {
//				odd=!odd;
//				while(pointsOnFrame.get(id+1).getY()==y&&pointsOnFrame.get(id).getX()+1==pointsOnFrame.get(id+1).getX())id++;
//				if(pointsOnFrame.get(id+1).equals(pointsOnFrame.get(id))) {
//					odd=!odd;
//					id++;
//					continue;
//				}
//				if(odd)startX=pointsOnFrame.get(id).getX();
//				else endX=pointsOnFrame.get(id).getX();
//				if(!odd) {
...
//			System.out.println(LineShape.Y1);
//			System.out.println(LineShape.Y2);
			while(id<pointsOnFrame.size()&&pointsOnFrame.get(id).getY()!=y)id++;
			if(id==pointsOnFrame.size()) {
				id=0;
				continue;
			}
			startX=pointsOnFrame.get(id).getX();
			while(pointsOnFrame.get(id).getY()==y) {
